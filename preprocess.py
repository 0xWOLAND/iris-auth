...
def load_dataset(
    ...
    if os.path.exists(cache_file):
        ...
        return tuple(np.load(cache_file)[k] for k in ['X1', 'X2', 'y'])
    
    def process_image(img_path):
        img = cv2.imread(img_path)
        seg = segmenter.extract(img)
        circles = detect_iris_hough(seg)
        if circles is None:
            return None
        return unwrap_iris(seg, 
    ...
    person_images = {
        pid: [img for img in [process_image(os.path.join(base_path, pid, f)) 
              for f in tqdm(os.listdir(os.path.join(base_path, pid)), desc=f"Person {pid}", leave=False) 
              if f.endswith('.jpg')] if img is not None]
        for pid in tqdm(os.listdir(base_path), desc="Processing persons") if pid.isdigit()
    }
